...
- checks: FE gate and CI `34119437100` PASS; BE gate and CI `34118960146` PASS; chart gate PASS; chart revision `7894c7f`; `kubectl -n argocd get application card-credit` reports `Succeeded Synced Healthy`; FE/BE deployments are 1/1; public `/login`, backend `/health` and `/ready` return HTTP 200.
- image evidence: FE 96,586,149 bytes pulled (down from 231,099,240); BE 77,915,569 bytes pulled after retaining `/shared`; compressed registry layer sums are 96,573,826 and 77,903,117 bytes respectively.
